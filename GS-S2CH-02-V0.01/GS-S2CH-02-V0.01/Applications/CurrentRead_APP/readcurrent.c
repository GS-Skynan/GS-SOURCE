...
void ReadCurrentInit(uint8_t* nfcData)
{
    ...
    if(power_Hold_1 >= 1200) power_Hold_1 = 1200;                                          // 限幅处理：功率保持值不能超过3500
    ...
    g_uPower =power_Hold_2;                                                              // 将处理后的功率保持值赋给当前通道电流变量
...
static uint16_t g_uTargetPower,g_uSetPower=600;
uint16_t Power_Compensation(void)
{

    g_uTargetPower=g_uSetPower-50-27;
