...
uint16_t power_Hold_1,power_Hold_2,power_Hold_3,power_Hold_4; 

uint8_t UART_REG1=0X00,UART_REG2=0X00,UART_REG3=0X00,UART_REG4=0X00;
uint8_t UART_REG1_Pre=0X00,UART_REG2_Pre=0X00,UART_REG3_Pre=0X00,UART_REG4_Pre=0X00;



uint8_t time_H,time_L;
uint16_t power_time=1,time2;             //2:表示0到1200W需要10s，其余功率按照比列进行
...
void ReadCurrentInit(uint8_t* nfcData)
{
    ...
    /*3通道*/
    power_Hold_3 = extractAndCombineEEPROMData(nfcData[10], nfcData[11]);  // 从I2C接收数据的第10和第11字节提取并组合成16位功率保持值
    if(power_Hold_3 >= 1200) power_Hold_3 = 1200;                                          // 限幅处理：功率保持值不能超过1200
    CURRENT_3 = power_Hold_3;                                                              // 将处理后的功率保持值赋给当前通道电流变量
    UART_REG3_Pre = UART_REG3 = nfcData[8];                                        // 从I2C接收数据的第8字节获取UART寄存器值，并保存当前值和前值
    if(UART_REG3 >= 0x64)UART_REG3 = 0x64;                                                 // 限幅处理：UART寄存器值不能超过0x64(十进制100)
    
    /*4通道*/
    power_Hold_4 = extractAndCombineEEPROMData(nfcData[14], nfcData[15]);  // 从I2C接收数据的第14和第15字节提取并组合成16位功率保持值
    if(power_Hold_4 >= 1200) power_Hold_4 = 1200;                                          // 限幅处理：功率保持值不能超过1200
    CURRENT_4 = power_Hold_4;                                                              // 将处理后的功率保持值赋给当前通道电流变量
    UART_REG4_Pre = UART_REG4 = nfcData[12];                                       // 从I2C接收数据的第12字节获取UART寄存器值，并保存当前值和前值
...
//    time11 = (unsigned int)((float)(4+power_time)/4.0f * (CURRENT_1 /2000.0f));            // 基于功率时间和通道1电流值的复杂计算，可能用于定时控制
    power_time = power_time*1000;                                                                    // 直接使用功率时间值作为time2  
}
