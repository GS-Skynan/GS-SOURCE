#include <stdint.h>
#include <stdio.h>
#include "../scl/DATA_DEAL.h"
#include "../../OTHER/scl/OTHER.h"
/**
...
{ 
    ...
    // 通道2（红外光）电流计算
    else if(channel == I_Out2) {
        // 获取通道2的ADC原始值
        I_Out2_ADC = ADC_Result2(I_Out2);        
        /* 电流转换公式分解：
    ...
    return (I_out * 1000);             // 统一转换为毫安(mA)单位
}
unsigned char str111[6];
void Data_Processing(void)
{   

   RS485();                         // 调用 RS485 函数，处理 RS485 通信数据

    Default_flag = 1;                // 设置 Default_flag 为 1，表示进入默认状态
    if(start_flag){
        // 检查是否有任意通道（1、2、3、4）的值大于等于 1
        if (UART_REG1 >= 0x1 || UART_REG2 >= 0x1 || UART_REG3 >= 0x1 || UART_REG4 >= 0x1) {    // 判断是否有通道开启
           if(pfc_flag) {
                PFC_SetHigh();                                                                     // 打开 PFC
            ...
            if (UART_REG1 >= 0x01 && out_flag1) {                               // 如果通道 1 的值大于等于 1
                ...
                JDQ_1_SetHigh();                                                // 打开通道 1 的继电器
                __delay_ms(100);                                                // 延时 150ms，等待继电器稳定
                L6562_SetLow();                                                 // 启动 L6562（PFC 控制器）
            }
            // 处理通道 2 的逻辑
            ...
            if(UART_REG1 >= 0x01 || UART_REG2 >= 0x01 ){
                ...
                if(UART_REG1 >= 0x01 && UART_REG2 >= 0x01 ){
                    Power_Compensation();
                    while ((UART_REG1 >= 0x01 && fabsf(TARGET_CURRENT_1_Real - get_current(I_Out1)) >= 4.5f  && V_Ret1 == 0 && out_flag1 && !return_flag1)  || (UART_REG2 >= 0x01 && fabsf(TARGET_CURRENT_2 - get_current(I_Out2)) >= 4.5f && out_flag2 && V_Ret2 == 0 ) ){
