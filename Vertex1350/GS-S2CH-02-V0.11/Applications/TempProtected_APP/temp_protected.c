...
void Temp_Protected(void)
{
    ...
    g_fActualTemperature = TemperatureVoltage;
    ...
    case 0: // 正常状态
        ...
        {
            if (TemperatureVoltage >= g_fOverTemperatureProtection+5 && TemperatureVoltage < g_fOverTemperatureProtection+10)
            {
                // 升到降功率阈值，开始计时
            ...
            else if (TemperatureVoltage >= g_fOverTemperatureProtection+10)
            {
                // 升到关闭阈值，开始计时
                if (NowTemperatureProtectionTime >= 1000)
                {
    ...
    case 1: // 降功率状态
        ...
        {
            if (TemperatureVoltage <= g_fOverTemperatureProtection)
            {
                // 降到正常阈值，延时返回
                if (NowTemperatureProtectionTime >= 1000)
                {
