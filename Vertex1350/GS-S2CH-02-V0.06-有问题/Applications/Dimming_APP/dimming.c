...
uint16_t time1 = 0;
volatile uint16_t PWM_CH1 = 0, PWM_CH2 = 0;
volatile float pwm_output1, pwm_output2;
volatile uint16_t powernum1, powernum2;
volatile float g_Voltage1, g_Voltage2;
volatile uint8_t PID_CH = 0;
...
float get_current(adc_channel_t channel)
{
    ...
    if (channel == OUT_CURRENT1)
    {
        ...
        I_Out1_ADC = ADC_Result(OUT_CURRENT1);
    ...
    {
        // 获取通道2的ADC原始值
        I_Out2_ADC = ADC_Result(OUT_CURRENT2);
...
void PIDCH12(void)
{
    ...
    if (PID_CH == 1 || PID_CH == 3)
    {
        g_Voltage1 = ((float) ADC_Result(Output1_voltage_ADC) / 1000.0f) * (1087.5f / 7.5f);
        powernum1 = (float) get_current(OUT_CURRENT1) * g_Voltage1 / 1000.0f;
        pwm_output1 = update_pwm_output_ch1(power_pwm);
        PWM_CH1 = PID_Compute(&pid1, pwm_output1, powernum1);
        PWM_Set_Direct(PWM_CHANNEL_1, PWM_CH1);
    }

    if (PID_CH == 2 || PID_CH == 3)
    {
        g_Voltage2 = ((float) ADC_Result(Output2_voltage_ADC) / 1000.0f) * (U2_R1 / U2_R2);
        powernum2 = (float) get_current(OUT_CURRENT2) * g_Voltage2 / 1000.0f;
        pwm_output2 = update_pwm_output_ch2(g_uChanne2Power);
        PWM_CH2 = PID_Compute(&pid2, pwm_output2, powernum2);
        PWM_Set_Direct(PWM_CHANNEL_2, PWM_CH2);
    }
}
...
void Display(void)
{
    printf("Vin:%d| \n\r", ADC_Result(Input_voltage_ADC));
    printf("CH1|I:%.2f|V:%.2f\n\r",get_current(OUT_CURRENT1), g_Voltage1);
    printf("CH2|I:%.2f|V:%.2f\n\r",get_current(OUT_CURRENT2), g_Voltage2);
    printf("PWM1:%d|\n\r",PWM_CH1); 
    printf("PWM2:%d|\n\r",PWM_CH2); 
    printf("P:%.2f|\n\r",powernum1); 
    printf("P:%.2f|\n\r",powernum2); 
