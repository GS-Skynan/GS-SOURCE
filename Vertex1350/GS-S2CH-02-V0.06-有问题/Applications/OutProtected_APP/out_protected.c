...
uint16_t Voltage_Judgment(adc_channel_t channel)
{
    ...
    if (channel == Output1_voltage_ADC && UART_REG1>0x01)
    {
        ...
        g_VoltageProtect1 = ((float) ADC_Result(Output1_voltage_ADC) / 1000.0f) * (1087.5f / 7.5f);
    ...
    {
        g_VoltageProtect2 = ((float) ADC_Result(Output2_voltage_ADC) / 1000.0f) * (U2_R1 / U2_R2);
        ...
        else if ((float)g_PoweProtect2 > (float)g_uPower2 * 1.10f)
        {
            g_uFaultCode = 6;
        //    printf("%.2f",g_PoweProtect2);
            return 2;
        }
        else
        {
