...
#define PWM_DUTY_CYCLE_CHANNEL1 2560.0f  //3.6A
#define PWM_DUTY_CYCLE_CHANNEL2 2560.0f
...
void PID_Init_Parameters(void)
{
    ...
    // 如果出现超调，增加Kd
    PID_Init(&pid1, 1, 0.08f, 0.002f, 0.15f, PWM_DUTY_CYCLE_CHANNEL1, 0.0f, 0);
    //  PID_Init(&pid2, 2, 0.1f, 0.0050f, 0.001f, 3200.0f, 0.0f, 0); // 第二路 PID 参数
    PID_Init(&pid2, 2, 0.3f, 0.0001f, 0.0001f, PWM_DUTY_CYCLE_CHANNEL2, 0.0f, 0);
...
uint16_t PID_Compute(PIDController *pid, float setpoint, float measured_value)
{
    ...
    if (pid->Channel_Flag == 1)
    {
        ...
        target_duty_cycle_Out1 += (int16_t) (output + (output >= 0 ? 0.5f : -0.5f));
        target_duty_cycle_Out1 = (uint16_t) fmaxf(0, fminf(target_duty_cycle_Out1, PWM_DUTY_CYCLE_CHANNEL1));
        return target_duty_cycle_Out1;
    }
    if (pid->Channel_Flag == 2)
    {
        ...
        target_duty_cycle_Out2 += (int16_t) (output + (output >= 0 ? 0.5f : -0.5f));
        target_duty_cycle_Out2 = (uint16_t) fmaxf(0, fminf(target_duty_cycle_Out2, PWM_DUTY_CYCLE_CHANNEL2));
        return target_duty_cycle_Out2;
    }
