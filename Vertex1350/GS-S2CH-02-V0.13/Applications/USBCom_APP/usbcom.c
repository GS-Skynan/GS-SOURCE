...
void Rs485Task(void)
{
    ...
    if (Rx_Buffer[0] == 0xAA && Rx_Buffer[1] == 0x5A)
    {
        switch (Rx_Buffer[3]) {
        case 0x01: //读写版本
            if (Rx_Buffer[2] == 0x01) Readhandle_version(Rx_Buffer);
            if (Rx_Buffer[2] == 0x02) Writehandle_version(Rx_Buffer);
            break;

        case 0x17: //Boot升级
            if (Rx_Buffer[2] == 0x02) Access_Bootloder(Rx_Buffer);
            break;

        case 0x57:
            if (Rx_Buffer[2] == 0x02) WriteCalibrationSingleChannel(Rx_Buffer);
            break;

        case 0x02:
            if (Rx_Buffer[2] == 0x01) ReadChannelPowerValue(Rx_Buffer);
            if (Rx_Buffer[2] == 0x02) WriteChannelPowerValue(Rx_Buffer);
            break;
