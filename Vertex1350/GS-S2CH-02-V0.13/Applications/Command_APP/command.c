...
// 错误码定义（可根据你的需求扩展）
typedef enum {
    ERROR_INVALID_ADDR_FUNC = 0x01,   // 地址码或功能码无效
    ERROR_PARAM_OUT_OF_RANGE = 0x02, // 参数值超范围
    ERROR_EXTERNAL_NO_RESPONSE = 0x03, // 外部设备无应答
    ERROR_UNKNOWN = 0xFF             // 未知错误
} ErrorCode_t;

void CommandErrorAck(ErrorCode_t error_code)
{
    // 创建错误应答指令的固定数据
    uint8_t error_response[17] = {
        0xAA, 0x5A, 0x01, 0xFF, 0x00, 0x04, 0x08, 0x00, // 第7个字节为错误码（初始为0）
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    };

    // 设置错误码（第7个字节，索引为7）
    error_response[7] = error_code;
...
uint8_t version[3] = {0x00, MAIN_VERSION, TEST_VERSION}; //版本号
...
void Readhandle_version(uint8_t *ack_data)
{
    ...
    read_buffer[11] = ver[1];
    read_buffer[12] = ver[2];
...
void Writehandle_version(uint8_t *w_data)
{
    ...
    // 读取版本信息
    uint8_t *ver = ReadNFCInfo(I2C_SOFTWARE_VERSION_ADDR, 3);
    write_buffer[11] = ver[1];
    write_buffer[12] = ver[2];
...
void WriteChannelPowerValue(uint8_t *w_data)
{
    ...
    {
        CommandErrorAck(ERROR_PARAM_OUT_OF_RANGE);
        return;
    }
...
{
    CommandErrorAck(ERROR_INVALID_ADDR_FUNC);
}
