...
uint16_t GetChannelVoltageValue(adc_channel_t channel)
{
    ...
    if (channel == Output1_voltage_ADC && g_uDimmingLevelChannel1 > 0x01)
    {
        ...
        if (g_VoltageProtect1 <= SHORT_CIRCUIT_CHANNEL1)
        {
            ...
            //CH1过压和过功率
        else if (g_VoltageProtect1 >= OVER_VOLTAGE_CHANNEL1 || (g_PoweProtect1 > OVER_POWER_CHANNEL1(g_uTargetPowerChannel1)))
        {
            g_uFaultCode = 2;
            return 3;
        }
            //CH1欠压
        else if (g_VoltageProtect1 <= UNDER_VOLTAGE_CHANNEL1)
        {
    ...
    if (channel == Output2_voltage_ADC && g_uDimmingLevelChannel2 > 0x01)
    {
        ...
        if (g_VoltageProtect2 <= SHORT_CIRCUIT_CHANNEL2)
        {
            ...
            //过压和过功率
        else if (g_VoltageProtect2 >= OVER_VOLTAGE_CHANNEL2 || (g_PoweProtect2 > OVER_POWER_CHANNEL2(g_uTargetPowerChannel2)))
        {
            g_uFaultCode = 5;
            return 3;
        }
            //欠压
        else if (g_VoltageProtect2 <= UNDER_VOLTAGE_CHANNEL2)
        {
            g_uFaultCode = 6;
            return 1;
        }
                          ...
                          uint8_t* enable_ptr)
{
    ...
    else if (outprotect == 3) // 电压异常类型3
    {
        if (!state->is_protecting3)
        {
            state->last_detect_time = current_time;
            state->is_protecting3 = true;
        }
        else
        {
            elapsed_time = get_elapsed_since(state->last_detect_time);
